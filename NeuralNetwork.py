...
class NeuralNetwork:
    ...
    def classify(self, dataFrame):
         ...
         self._mlp = self._mlp.fit(x_train, y_train)

         y_pred = self._mlp.predict(x_test)

         self.printStats(y_test, y_pred, "Neural Network")
    ...
    def getLearningCurve(self,dataFrame):
        ...
        plt.title("Learning Curve for a Neural Network Model", fontsize = 18, y = 1.03)
        plt.legend()
        plt.gca().invert_yaxis()
        
       
        plt.savefig("NeuralNetworkLearningCurve.png")
        plt.show()
